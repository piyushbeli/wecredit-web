...
/**
 * FAQ Section Component for Personal Loan Page
 * Displays frequently asked questions about personal loans using accordion UI
 */

import { JSX, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { ChevronDown } from 'lucide-react';
import { PERSONAL_LOAN_FAQS, type FaqItem } from './constants';
...
const FaqAccordionItem = ({
  ...
  onToggle,
  index,
}: FaqAccordionItemProps): JSX.Element => {
  return (
    <motion.div
  ...
  return (
    <section className="bg-white py-4 sm:py-10 md:py-12 px-4">
      ...
      <div className="max-w-2xl mx-auto space-y-1">
        {PERSONAL_LOAN_FAQS.map((item, index) => (
          <FaqAccordionItem
            key={item.id}
            item={item}
