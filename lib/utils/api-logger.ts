  ...
  if (isProduction) {
    const logData = {
      ...
      request: {
        ...
        body: metadata.body,
        mobile: sanitizeMobile(metadata.mobile),
        hasAuthorization: metadata.hasAuthorization,
      },
    };
...
function logSuccess(endpoint: string, metadata: ApiResponseMetadata): void {
  if (isProduction) {
    ...
    const statusEmoji = metadata.statusCode >= 200 && metadata.statusCode < 300 ? '✅' : '⚠️';
    console.group(`🌐 [API] ${endpoint}`);
    console.log('📩 RESPONSE');
    console.log(`← STATUS: ${statusEmoji} ${metadata.statusCode} (${metadata.duration}ms)`);
